...
public abstract class AbstractConfiguration {
    ...
    private static String nameTKey(final String key) {
        final String tKey = currentCategory.isEmpty()
                ? String.format("mod.%s.config.%s", ChiselsAndBits.MODID, key)
                : String.format("mod.%s.config.%s.%s", ChiselsAndBits.MODID, currentCategory, key);
    ...
    private static Builder buildBase(final Builder builder, final String key) {
        return builder.comment(DeprecationHelper.translateToLocal(commentTKey(key)))
                .translation(nameTKey(key));
    }
    ...
    protected static IntValue defineInteger(
            final Builder builder, final String key, final int defaultValue, final int min, final int max) {
    ...
    protected static LongValue defineLong(
            final Builder builder, final String key, final long defaultValue, final long min, final long max) {
        return buildBase(builder, key).defineInRange(key, defaultValue, min, max);
    }
    ...
    protected static DoubleValue defineDouble(
            final Builder builder, final String key, final double defaultValue, final double min, final double max) {
        return buildBase(builder, key).defineInRange(key, defaultValue, min, max);
    }
    ...
    protected static <V extends Enum<V>> EnumValue<V> defineEnum(
            final Builder builder, final String key, final V defaultValue) {
        return buildBase(builder, key).defineEnum(key, defaultValue);
    }

    protected static ConfigValue<String> defineString(
            final Builder builder, final String key, final String defaultValue) {
        return buildBase(builder, key).define(key, defaultValue);
    }
}
