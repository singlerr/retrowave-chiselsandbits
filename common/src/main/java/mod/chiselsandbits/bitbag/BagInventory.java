...
public class BagInventory implements IInventory {
    ...
    @Override
    public @Nonnull ItemStack getStackInSlot(final int index) {
        final int qty = inv.contents[ItemBitBag.INTS_PER_BIT_TYPE * index + ItemBitBag.OFFSET_QUANTITY];
        final int id = inv.contents[ItemBitBag.INTS_PER_BIT_TYPE * index + ItemBitBag.OFFSET_STATE_ID];
    ...
    @Override
    public ItemStack decrStackSize(final int index, int count) {
        final int qty = inv.contents[ItemBitBag.INTS_PER_BIT_TYPE * index + ItemBitBag.OFFSET_QUANTITY];
        final int id = inv.contents[ItemBitBag.INTS_PER_BIT_TYPE * index + ItemBitBag.OFFSET_STATE_ID];
    ...
    public List<ITextComponent> listContents(final List<ITextComponent> details) {
        ...
        for (final Entry<String, Integer> e : list) {
            details.add(new StringTextComponent(e.getValue().toString())
                    .appendString(" ")
                    .appendString(e.getKey()));
        }
